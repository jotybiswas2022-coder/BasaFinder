@extends('admin.layouts.app')

@push('styles')
<style>
@keyframes fadeUp {
    from { opacity: 0; transform: translateY(16px); }
    to   { opacity: 1; transform: translateY(0); }
}
.tl-animate { animation: fadeUp 0.4s ease-out both; }

.tl-hero {
    position: relative;
    margin: -2rem -2rem 1.5rem;
    padding: 2rem 2.5rem;
    background: linear-gradient(135deg, #0b1120 0%, #1a1a2e 40%, #16213e 70%, #0f3460 100%);
    overflow: hidden;
    isolation: isolate;
}
.tl-hero .hero-inner {
    position: relative; z-index: 1;
    display: flex; align-items: center; justify-content: space-between; gap: 1.5rem;
}
.tl-hero .hero-content { flex: 1; min-width: 0; }
.tl-hero h1 { color: #fff; font-size: 1.375rem; font-weight: 800; letter-spacing: -0.03em; }
.tl-hero p { color: rgba(255,255,255,0.5); font-size: 0.8125rem; margin-top: 0.125rem; }
.tl-hero .hero-btn {
    display: inline-flex; align-items: center; gap: 0.375rem;
    padding: 0.5rem 1.125rem; background: #6366f1; color: #fff;
    border-radius: 0.5rem; font-size: 0.8125rem; font-weight: 600;
    text-decoration: none; transition: all 0.2s; white-space: nowrap;
}
.tl-hero .hero-btn:hover { background: #4f46e5; transform: translateY(-1px); }

.faq-list { display: flex; flex-direction: column; gap: 0.625rem; }
.faq-row {
    display: flex; align-items: center; gap: 0.75rem;
    background: #fff; border: 1px solid #e5e7eb; border-radius: 0.625rem;
    padding: 0.75rem 1rem; transition: all 0.2s;
}
.faq-row:hover { border-color: #c7d2fe; box-shadow: 0 1px 4px rgba(99,102,241,0.06); }
.faq-order {
    width: 2rem; text-align: center; font-size: 0.75rem; font-weight: 700;
    color: #9ca3af; flex-shrink: 0;
}
.faq-body { flex: 1; min-width: 0; }
.faq-body .q { font-weight: 600; font-size: 0.875rem; color: #111827; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.faq-body .a { font-size: 0.8rem; color: #9ca3af; margin-top: 0.125rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.faq-badge {
    display: inline-flex; align-items: center; padding: 0.15rem 0.5rem;
    border-radius: 9999px; font-size: 0.6875rem; font-weight: 600; flex-shrink: 0;
}
.faq-badge.on { background: #d1fae5; color: #065f46; }
.faq-badge.off { background: #fef3c7; color: #92400e; }
.faq-actions { display: flex; gap: 0.25rem; flex-shrink: 0; }
.faq-actions a, .faq-actions button {
    display: inline-flex; align-items: center; gap: 0.25rem;
    padding: 0.3rem 0.625rem; border-radius: 0.375rem;
    font-size: 0.6875rem; font-weight: 600; cursor: pointer;
    text-decoration: none; border: none; transition: all 0.15s;
}
.faq-edit { background: #eef2ff; color: #4f46e5; }
.faq-edit:hover { background: #e0e7ff; }
.faq-delete { background: #fef2f2; color: #dc2626; }
.faq-delete:hover { background: #fee2e2; }

.tl-empty {
    text-align: center; padding: 3rem 1rem; color: #9ca3af;
}
.tl-empty svg { margin-bottom: 0.75rem; opacity: 0.3; }
.tl-empty h3 { font-size: 1rem; font-weight: 600; color: #6b7280; margin-bottom: 0.25rem; }
.tl-empty p { font-size: 0.8125rem; }
</style>
@endpush

@section('content')
<div class="tl-hero tl-animate">
    <div class="hero-inner">
        <div class="hero-content">
            <h1>FAQs</h1>
            <p>Manage frequently asked questions.</p>
        </div>
        <a href="{{ route('admin.faqs.create') }}" class="hero-btn">
            <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            Add FAQ
        </a>
    </div>
</div>

@if(session('success'))
    <div style="padding:0.75rem 1rem; background:#d1fae5; color:#065f46; border-radius:0.5rem; margin-bottom:1rem; font-size:0.8125rem; font-weight:500; display:flex; align-items:center; gap:0.5rem;">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
        {{ session('success') }}
    </div>
@endif

<div class="faq-list tl-animate" style="animation-delay:0.1s">
    @forelse($faqs as $f)
        <div class="faq-row">
            <span class="faq-order">{{ $f->sort_order }}</span>
            <div class="faq-body">
                <div class="q">{{ $f->question }}</div>
                <div class="a">{{ $f->answer }}</div>
            </div>
            <span class="faq-badge {{ $f->is_active ? 'on' : 'off' }}">{{ $f->is_active ? 'Active' : 'Inactive' }}</span>
            <div class="faq-actions">
                <a href="{{ route('admin.faqs.edit', $f->id) }}" class="faq-edit">Edit</a>
                <form method="POST" action="{{ route('admin.faqs.destroy', $f->id) }}" onsubmit="return confirm('Delete this FAQ?')" style="margin:0;">
                    @csrf @method('DELETE')
                    <button type="submit" class="faq-delete">Delete</button>
                </form>
            </div>
        </div>
    @empty
        <div class="tl-empty">
            <svg width="40" height="40" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
            <h3>No FAQs yet</h3>
            <p>Get started by adding your first FAQ.</p>
        </div>
    @endforelse
</div>

@if($faqs->hasPages())
    <div style="margin-top:1.5rem;">
        {{ $faqs->links('vendor.pagination.custom') }}
    </div>
@endif
@endsection
