...
.tl-content-cell { max-width: 320px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #6b7280; }
.tl-status {
...
<div class="tl-hero tl-animate">
    <div class="hero-inner">
        <div class="hero-content">
            <h1>FAQs</h1>
            <p>Manage frequently asked questions displayed on the home page.</p>
        </div>
    ...
    @if($faqs->count())
        <table class="tl-table">
            ...
            <tbody>
                @foreach($faqs as $f)
                    <tr>
                        <td style="color:#9ca3af;">{{ $f->sort_order }}</td>
                        <td style="font-weight:600;color:#111827;">{{ $f->question }}</td>
                        <td><span class="tl-content-cell">{{ $f->answer }}</span></td>
                        <td>
                            <span class="tl-status {{ $f->is_active ? 'active' : 'inactive' }}">
                                {{ $f->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td>
                            <div class="tl-actions" style="justify-content:flex-end;">
                                <a href="{{ route('admin.faqs.edit', $f->id) }}" class="tl-btn-edit">
